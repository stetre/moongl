...
local TITLE = "Chapter 8 - Simple shadow map"
local W, H = 800, 600
...
local prog, vsh, fsh = gl.make_program({vertex="shaders/shadowmap.vert", 
                                       fragment="shaders/shadowmap.frag"})
gl.delete_shaders(vsh, fsh)
local prog1, vsh, fsh = gl.make_program({vertex="shaders/solid.vert", 
...
-- Generate the meshes
local plane = new_plane(40, 40, 2, 2)
local teapot = new_teapot(14)
local torus = new_torus(0.7*2, 0.3*2, 50,50)
...
local shadowBias = mat4(.5, 0, 0, .5,
                         ...
                         0, 0,.5, .5,
                         0, 0, 0,  1)
local lightPos = vec3(0.0,1.65*5.25,1.65*7.5)  -- World coords
...
local function draw_scene(view, projection)
   local color = vec3(0.7,0.5,0.3)
   set_material(color*.05, color, {.9, .9, .9}, 150.0)
   local model = rotate(rad(-90), 1, 0, 0)
   set_matrices(model, view, projection)
   teapot:render()

   set_material(color*.05, color, {.9, .9, .9}, 150.0)
   model = translate(0.0,2.0,5.0)*rotate(rad(-45), 1,0,0)
   set_matrices(model, view, projection)
   torus:render()

   set_material({0.05, 0.05, 0.05}, {0.25, 0.25, 0.25}, {0.0, 0.0, 0.0}, 1.0)
   model = mat4()
   set_matrices(model, view, projection)
   plane:render();

   model = translate(-5.0,5.0,0.0)*rotate(rad(-90), 0, 0, 1)
   set_matrices(model, view, projection)
   plane:render();

   model = translate(0.0,5.0,-5.0)*rotate(rad(90), 1, 0, 0)
   set_matrices(model, view, projection)
   plane:render();
end
...
while not glfw.window_should_close(window) do
   ...
   -- Pass 2 (render)
   local c = 2.0
   local cameraPos = vec3(c*11.5*cos(angle), c*7.0, c*11.5*sin(angle))
   local view = glmath.look_at(cameraPos, vec3(0,0,0), vec3(0,1,0))
   ...
   gl.uniformf(loc1["Color"], 1.0, 0.0, 0.0, 1.0)
   gl.uniform_matrix4f(loc1["MVP"], true, projection*view*frustum:inverse_view())
   frustum:render()
