...
local TITLE = "Chapter 10 - Particle system simulating fire"
local W, H = 800, 600
...
gl.shader_source(vsh, readsource("shaders/fire.vert"))
gl.compile_shader(vsh, true)
gl.attach_shader(prog, vsh)
local fsh = gl.create_shader('fragment')
gl.shader_source(fsh, readsource("shaders/fire.frag"))
...
local particleLifetime = 3.0
local nParticles = 2000
local emitterPos = vec3(0, 0, 0)
local emitterDir = vec3(0, 1, 0)

-- Load the particle texture
gl.active_texture(0)
local texid = texture.load_texture("../media/texture/fire.png")
gl.bind_texture_unit(0, texid)
-- Load the random texture
gl.active_texture(1)
local texid = texture.random_1d(nParticles*2)
...
gl.uniformf(loc["ParticleSize"], 0.5)
gl.uniformf(loc["Accel"], 0.0, 0.1, 0.0)
...
gl.uniformf(loc1["Color"], 0.2, 0.2, 0.2, 1.0)
