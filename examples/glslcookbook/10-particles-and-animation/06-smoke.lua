...
local TITLE = "Chapter 10 - Particle system simulating smoke"
local W, H = 800, 600
...
gl.shader_source(vsh, readsource("shaders/smoke.vert"))
gl.compile_shader(vsh, true)
gl.attach_shader(prog, vsh)
local fsh = gl.create_shader('fragment')
gl.shader_source(fsh, readsource("shaders/smoke.frag"))
...
gl.active_texture(0)
local texid = texture.load_texture("../media/texture/smoke.png")
...
gl.uniformf(loc1["Color"], 0.5, 0.5, 0.5, 1.0)
