...
local prog0, vsh, fsh = gl.make_program({ -- shader
   vertex="shaders/3.1.3.shadow_mapping.vert",
   fragment="shaders/3.1.3.shadow_mapping.frag",
})
gl.delete_shaders(vsh, fsh)
local prog, vsh, fsh = gl.make_program({ -- simpleDepthShader
   vertex="shaders/3.1.3.shadow_mapping_depth.vert",
   fragment="shaders/3.1.3.shadow_mapping_depth.frag",
})
gl.delete_shaders(vsh, fsh)
local prog1, vsh, fsh = gl.make_program({ -- debugDepthQuad
   vertex="shaders/3.1.3.debug_quad.vert",
   fragment="shaders/3.1.3.debug_quad_depth.frag",
})
gl.delete_shaders(vsh, fsh)
...
gl.texture_parameter('2d', 'border color', {1.0, 1.0, 1.0, 1.0})
...
local moving_light, moving_light_key_pressed = false, false
...
print("Press space to toggle moving light")
...
while not glfw.window_should_close(window) do
   ...
   if keypressed('space') and not moving_light_key_pressed then
      moving_light = not moving_light
      moving_light_key_pressed = true
   end
   if keyreleased('space') then moving_light_key_pressed = false end
   -- render
   gl.clear_color(0.1, 0.1, 0.1, 1.0)

   if moving_light then
      light_pos = vec3(3*sin(t), 2*cos(t), 5+cos(t))
   end
