...
local prog1 = make_program("8.2.g_buffer.vert", "8.2.g_buffer.frag")
local prog2 = make_program("8.2.deferred_shading.vert", "8.2.deferred_shading.frag")
local prog3 = make_program("8.2.deferred_light_box.vert", "8.2.deferred_light_box.frag")
make_program = nil
...
for i=1, NR_LIGHTS do
   ...
   table.insert(uniforms, "lights["..(i-1).."].Radius")
...
while not glfw.window_should_close(window) do
   ...
   for i = 1, NR_LIGHTS do
      ...
      local linear, quadratic = 0.7, 1.8
      local maxBrightness = max(lightColors[i].r, lightColors[i].g, lightColors[i].b)
      local radius = (-linear+sqrt(linear*linear-4*quadratic*(constant-(256/5)*maxBrightness)))
                        /(2*quadratic)
      gl.uniformf(loc2["lights["..(i-1).."].Radius"], radius)
      gl.uniformf(loc2["lights["..(i-1).."].Linear"], linear)
      gl.uniformf(loc2["lights["..(i-1).."].Quadratic"], quadratic)
   end
