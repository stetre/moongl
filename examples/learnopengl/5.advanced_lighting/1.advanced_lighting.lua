...
local prog, vsh, fsh = gl.make_program({
   vertex="shaders/1.advanced_lighting.vert",
   fragment="shaders/1.advanced_lighting.frag",
})
...
-- load textures
local floor_texture = new_texture("../resources/textures/wood.png")
...
local uniforms = -- names of prog's uniform variables
   { "projection", "view", "viewPos", "lightPos", "blinn" }
gl.use_program(prog)
...
local blinn, blinn_key_pressed = false, false
...
-- render loop
print("Press 'b' to toggle shading model")
while not glfw.window_should_close(window) do
   ...
   if keypressed('b') and not blinn_key_pressed then
      blinn = not blinn
      blinn_key_pressed = true
   end
   if keyreleased('b') then blinn_key_pressed = false end
   glfw.set_window_title(window, "LearnOpenGL - "..(blinn and "Blinn-Phong" or "Phong"))
   ...
   gl.uniformi(loc.blinn, blinn and 1 or 0)
   -- floor
